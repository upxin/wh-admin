...
import { getUser,delUser,updateUser ,listUser,addUser} from "@@/apis/user/index"
import { usePagination } from "@@/composables/usePagination"
import { CirclePlus, Delete, Download, Refresh, RefreshRight, Search , Upload } from "@element-plus/icons-vue"
import { cloneDeep } from "lodash-es"
type TableData = {userName:string, userId:string,role:string}
defineOptions({
  // 命名当前组件
  name: "ElementPlus"
})
...
const formRules: FormRules<TableData> = {
  ...
  role: [{ required: true, trigger: "blur", message: "请输入手机" }],
}
...
<template>
  <div class="app-container">
    ...
    <!-- 新增/修改 -->
    <el-dialog v-model="dialogVisible" :title="!formData.userId ? '新增用户' : '修改用户'" width="500px"
      @closed="resetForm">
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" label-position="left">
        <el-form-item prop="userName" label="用户名">
          <el-input v-model="formData.userName" placeholder="请输入" />
        </el-form-item>
        <el-form-item prop="role" label="手机号">
